...
async function main() {
  ...
  // Withdraw stake.
  let withdrawStakeTx = StakeProgram.withdraw({
    stakePubkey: stakeAccount.publicKey,
    authorizedPubkey: wallet.publicKey,
  ...
  let withdrawStakeTxId = await sendAndConfirmTransaction(
    connection,
    withdrawStakeTx,
    [ wallet ]
  );

  console.log('withdrawStakeTxId: ', withdrawStakeTxId);

  stakeAccountBalance = await connection.getBalance(stakeAccount);
